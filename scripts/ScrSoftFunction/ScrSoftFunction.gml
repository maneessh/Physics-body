...
function InitSoftBox(_sb, _x , _y ,_mass = 1, _stiffness = 2, _iterations = 4)
{
    with (_sb ) {
        ...
        AddSoftStick(self.id, p_tl, p_tr ,_stiffness);
        AddSoftStick(self.id, p_tr, p_br , _stiffness);
        AddSoftStick(self.id, p_br, p_bl , _stiffness);
        AddSoftStick(self.id, p_bl, p_tl , _stiffness);
        
        var _brace = AddSoftStick(self.id, p_tl, p_br, _stiffness);
        _brace.visible = false;
    }
}

function InitSoftBall(_sb, _x , _y , _radius, _mass = 1, _stiffness = 1, _point_count = 12 , _iterations = 6 )
{
    with (_sb) {
        ...
        for (var i = 0; i < _point_count; i++) 
        {
            ...
            AddSoftStick(self.id, _p1 , _p2 , _stiffness);
        }
        ...
        for (var i = 0; i < _point_count / 2; i++) 
        {
            ...
            var _brace = AddSoftStick(self.id, _p1 , _p2, _stiffness);
...
function AddSoftStick(_sb, _p1 , _p2, _stiffness)
{
    var _s = {
        point1 : _p1,
        point2 : _p2,
        ...
        repeat (iterations) {
            for (var i = 0; i < ds_list_size(sticks); i++) {
                ...
                if (_dist = 0 ) continue ; //Avoid div by Zero
                 ...
                 // 0 = no correction, 1 = fully rigid (your current behavior)


                //How much correction is required ? Why /2 ? moving both points.
                var _diff = (_s.length - _dist) / _dist * _s.stiffness;
